...
<section id="<?php echo esc_attr($block_id); ?>" class="hero-video-slider js-hero-video-slider">
    <div class="hero-video-slider__stage">
        <?php foreach ($slides as $index => $slide) :
            ...
            $slide_cta_url = is_array($slide_cta) ? ($slide_cta['url'] ?? '') : '';
            $slide_cta_title = is_array($slide_cta) ? ($slide_cta['title'] ?? __('Scopri', 'filcar')) : __('Scopri', 'filcar');
            $slide_cta_target = is_array($slide_cta) && !empty($slide_cta['target']) ? $slide_cta['target'] : '_self';
        ?>
